...
class listprd extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['topsale'] = $this->topsale_model->getTopFive();
		...
		$this->load->view('listprd/mainpage2', $data);
		$this->load->view('template/frontfooter');
	}



	public function read($img_id)

	{

		//print_r($img_id);
	    //eixt();
		$data['result'] = $this->listprd_model->read_listprd($img_id);  
		//print_r($data['result']);

		//echo $data['result']->img_detail;
		//exit;
		$this->load->view('template/frontheader');
		$this->load->view('listprd/prddetail',$data);
		$this->load->view('template/frontfooter');
	}



	public function booking($img_id)

	{

		//print_r($img_id);
	    //eixt();
		$data['result'] = $this->listprd_model->read_booking($img_id);  
		//print_r($data['result']);

		//echo $data['result']->img_detail;
		//exit;
		$this->load->view('template/frontheader');
		$this->load->view('listprd/booking',$data);
		$this->load->view('template/frontfooter');
	}
	...
	public function dobooking($value='')
	{
		...
		//exit();
                        ...
                        $this->db->insert('booksdetail',$arr);

    //                     $this->session->set_flashdata(
				// 	array(
				// 		'msginfo2'=>'<div class="pad margin no-print"><div style="margin-bottom: 0!important;" class="callout callout-info"><h4><i class="fa fa-info"></i> ข้อความจากระบบ</h4>ทำรายการสำเร็จ</div></div>'
				// 	)
				// );
 				     //redirect("javascript:history.back(-1);");
 //echo "<script>window.location.href='javascript:history.back();'</script>";
                       // exit;	
                       redirect('order/m_order', 'refresh');
